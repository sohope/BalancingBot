...
data = {
    ...
    'AN': deque(maxlen=MAX_POINTS),
    'PO': deque(maxlen=MAX_POINTS),
}

latest = {'AX': 0, 'AY': 0, 'AZ': 0, 'GX': 0, 'GY': 0, 'GZ': 0, 'AN': 0, 'PO': 0}
...
# ── 파싱 ──────────────────────────────────────────────
def parse_line(line):
    """AX:0.01 AY:-0.02 AZ:1.00 GX:0.3 GY:-0.1 GZ:0.0 AN:3.2 PO:1.5"""
    try:
...
app.layout = html.Div(style={
    ...
    html.Div(style={
        ...
        make_indicator('Angle', 'AN', 'deg'),
        make_indicator('PID Out', 'PO', '%'),
    ]),
    ...
    html.Div(style={'backgroundColor': COLORS['panel'], 'borderRadius': '8px',
        ...
        dcc.Graph(id='accel-graph', style={'height': '250px'},
    ...
    html.Div(style={'backgroundColor': COLORS['panel'], 'borderRadius': '8px',
        ...
        dcc.Graph(id='gyro-graph', style={'height': '250px'},
                  config={'displayModeBar': False}),
    ]),
    ...
    html.Div(style={'backgroundColor': COLORS['panel'], 'borderRadius': '8px',
                     'padding': '12px 16px', 'marginBottom': '16px'}, children=[
        dcc.Graph(id='angle-graph', style={'height': '250px'},
                  config={'displayModeBar': False}),
    ]),
    ...
    html.Div(style={'backgroundColor': COLORS['panel'], 'borderRadius': '8px',
        ...
        dcc.Graph(id='pid-graph', style={'height': '250px'},
                  config={'displayModeBar': False}),
    ]),
...
@app.callback(
    ...
    Output('ind-AN', 'children'),
    Output('ind-PO', 'children'),
...
def update(_, accel_axes, gyro_axes):
    ...
    # Angle 그래프
    angle_fig = go.Figure(layout=make_graph_layout('Complementary Filter Angle', 'deg'))
    angle_fig.add_trace(go.Scatter(
        x=t, y=list(data['AN']), name='Angle',
        line=dict(color=COLORS['AN'], width=2),
    ))
    angle_fig.add_hline(y=0, line_dash='dash', line_color='#6b7280', line_width=1)

    # PID Output 그래프
    pid_fig = go.Figure(layout=make_graph_layout('PID Output', '%'))
    pid_fig.add_trace(go.Scatter(
        x=t, y=list(data['PO']), name='PID Output',
        line=dict(color=COLORS['PO'], width=2),
    ))
    pid_fig.add_hline(y=0, line_dash='dash', line_color='#6b7280', line_width=1)

    # 현재 값 텍스트
    indicators = []
    for key in ['AX', 'AY', 'AZ', 'GX', 'GY', 'GZ', 'AN', 'PO']:
        indicators.append(f'{latest[key]:.1f}')

    return accel_fig, gyro_fig, angle_fig, pid_fig, *indicators
